...
# 构建MySQL的数据库Util工具
# 这个需求涉及到：元数据，数据源，目的地 三个数据库， 我们构建3个db_util工具
# 构建元数据库的util对象
...
if not source_db_util.check_table_exists(conf.source_db_name,conf.source_barcode_data_table_name):
    ...
    sys.exit(1)     # sys.exit()表示python程序停止运行，exit(0)表示是正常的停止，传入非0是异常的停止
# 接着判断，目的地的MySQL库中，是否有我们要写入的表，如果没有，那么就要新建了
...
# 开始查询的流程了
# 注意：每一次查询，都要去判断一下update_at, 来决定本次查询从哪个时间开始
...
if not metadata_db_util.check_table_exists(conf.metadata_db_name, conf.metadata_barcode_table_name):
    ...
    result = metadata_db_util.querry(query_sql)
    # 表存在，不代表里面有数据
    if len(result) != 0:
        # 代表有表，并且查询数据
...
# 执行SQl查询
result = source_db_util.querry(sql)
# pymysql的查询结果是：((结果的某行),(结果的某行)，(结果的某行)....)
# (结果的某行) 是：元组类型，里面存储的是查询结果的列，比如：（1.'张三'，11）...
# TODO: 步骤3：开始准备构建模型了构建 Insert INTO 插入语句
# 构建模型
...
for single_line_result in result:
    ...
    # 查询是select * 查询，结果中的第一个列 第二个列 第N个列是谁，和数据库 的表结构顺序一致
...
for model in barcode_models:
    ...
    # 执行插入,使用execute_without_autocommit 配合autocommit 为False应用批量提交提高性能

...
class OrderModel:
    ...
    def to_csv(self,sep=","):
        '''
        将此模型对象，转换成一条CSV格式的字符串，以参数(seq)传入的符号作为分隔符
        :param sep:分隔符，默认是逗号
    ...
    def generate_insert_sql(self):
        """
        将模型转换成一条INSET SQl语句
        :return: 字符串，记录了 INSERT INTO的SQL语句
        """
...
class OrdersDetailModel:
    ...
    def __init__(self,data):
        ...
        # 记录当前订单内，售卖的全部商品信息，每一条数据是一个SingleProductSoldModel对象
        self.products_detail = []
        # 从字典中取出商品信息的list
        order_products_list = data_dict["product"] # 这个product列已经包含了json数据中的product这一字典的列表
    ...
    def generate_insert_sql(self):
        ...
        for model in self.products_detail:
            # model是类：SingleProductSoldModel的一个对象
            sql += "("
...
class SingleProductSoldModel:
    ...
    def __init__(self, order_id, product_detail_dict):
        ...
        self.name = product_detail_dict["name"]                 # 商品名字
